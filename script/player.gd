...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("left_mouse") and bow_equiped and bow_cooldwn:
		bow_cooldwn = false
...
func play_anim(dir):
	...
	if bow_equiped:
		...
		if mouse_lec_from_player.x >= 25 and mouse_lec_from_player.y <= -25:
			$AnimatedSprite2D.play("ne-attack")
		if mouse_lec_from_player.x >= 0.5 and mouse_lec_from_player.y >= 25:
